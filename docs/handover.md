...
- **우선순위**: 실질 버그(런타임 오작동)를 최우선으로 본다. 스타일/취향은 비블로킹으로 취급하되, 컨벤션 위반은 일관성 차원에서 요청한다.
- **접근성은 하드룰**: 로딩·상태 표시에 `role=status`/`aria-live`/`sr-only` 등 접근성 속성이 유지되는지 확인한다.
- **재사용 우선(과설계 금지)**: 새로 만들기 전에 기존 공용 컴포넌트·유틸을 찾아 재사용한다. 전역 상태/추상화가 단일 호출자만을 위한 것이면 되묻는다.
- **전역 변경은 영향 범위를 문서화**: 공용 컴포넌트(예: `Button`) 변경은 영향받는 사용처를 PR 본문에 표로 정리한다.
- **비블로킹은 follow-up 이슈로 분리**한다. PR 범위를 좁게 유지하고, 무관한 변경은 별도 PR/이슈로 뺀다.
- **봇 활용**: `@coderabbitai review`로 자동 리뷰를 돌리고, 지적은 현재 코드 기준으로 검증해 유효한 것만 반영하고 나머지는 근거와 함께 리졸브한다.
- **머지는 리뷰어가** 수행한다.
- 이슈/PR 제목: 백엔드 `[Feat]/[Fix]/[Chore] 설명`, 프론트 `[Feat]/[Fix] #이슈번호 - 설명`. 라벨은 `bug`/`enhancement` 등 기존 라벨.
