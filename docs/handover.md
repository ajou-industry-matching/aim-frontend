...
이슈와 PR 논의에서 실제로 내려진 결정과 그 이유를 정리한 것이다. 새로 합류한 사람이 "왜 이렇게 되어 있는가"를 되묻지 않도록 남긴다.

- **상세 라우팅을 동적 `[id]` 대신 CSR + 쿼리 파라미터(`?id=`)로 구현한다.** 정적 export(`output: "export"`)는 빌드 시점에 모든 id를 알아야 하고, 사전 생성되지 않은 경로는 404가 난다. 그래서 클라이언트 컴포넌트에서 데이터를 패칭하는 패턴을 택했다. (aim-frontend #124, #127)
- **홈/포트폴리오 구현 PR에서 관리자 섹션과 공지 페이지 스코프를 빼고 별도로 분리한다.** 한 PR에 여러 기능이 섞이는 것을 막고 충돌 없이 머지하기 위해, 연관성 높은 범위만 남기고 스코프를 재조정했다. (aim-frontend #120)
- **크롤러가 운영 DB에 직접 INSERT하던 방식을 백엔드 전용 API(`POST /api/crawled-projects`) 전송으로 바꾼다.** 데이터 관리 주체를 백엔드로 통일하고 인증 경로를 확립하기 위함이다. 크롤러 시스템 유저의 `admin_role` 저장 값도 정수에서 백엔드 문자열 enum과 정합화했다. (aim-crawler #9, #8, aim-backend #49)
- **공지 작성 API를 JSON이 아니라 인증 필수 + `multipart/form-data`로 맞추고 작성 기능을 분리한다.** 백엔드 계약과 프론트 전송 방식이 어긋나 등록이 항상 실패하던 문제를 해결했다. 리치 에디터 이미지 Base64 인코딩으로 인한 저장 용량 한도 초과 문제는 별도 이슈로 분리했다. (aim-frontend #127, #179, 이미지 이슈 #187)
- **소유권 판정 로직을 고치고 상세 응답에 누락 필드 추가를 백엔드에 요청한다.** `/api/users/me`(`UserMeResponse`)에 `userId`가 없어 작성자 판정이 안 되어 수정 버튼이 숨겨졌다. 공지 상세(`PostDetailResponse`)에는 `authorName`이 없어 작성자 표시가 안 되던 비대칭도 함께 정리했다. (aim-frontend #133, #179)
- **Cloud Run과 DB 관련 리전을 `asia-northeast3`로 통일하고 컷오버 순서를 문서화한다.** DB는 서울 리전인데 앱은 미국 리전이라 조회 쿼리당 약 180ms 왕복 지연이 있었다. 컷오버는 백엔드를 새 리전에 먼저 배포한 뒤 프론트 rewrite를 전환하는 순서를 지켜야 한다. (aim-backend #41·#45, aim-frontend #191)
